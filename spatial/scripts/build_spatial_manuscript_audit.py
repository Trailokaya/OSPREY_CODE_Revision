...
def write_report(
    ...
    lines = [
        ...
        "",
        f"Permutation Moran's I uses {N_PERMUTATIONS} deterministic permutations per sampled time window with seed {AUDIT_SEED}. Hourly and daily windows are deterministically thinned when needed to keep the audit reproducible and computationally bounded; weekly, monthly, and total-period windows are kept in full.",
        "",
    ...
    (OUTPUT_DIR / "spatial_manuscript_audit.md").write_text("\n".join(lines) + "\n")
...
def main() -> None:
    ...
    metadata = {
        "purpose": "Manuscript-oriented spatial audit: distance-band support, distance-decay thresholds, permutation Moran sensitivity, and Chicago raw high-resolution inventory.",
        "seed": AUDIT_SEED,
        "n_permutations": N_PERMUTATIONS,
        "alpha": ALPHA,
    ...
    (OUTPUT_DIR / "spatial_manuscript_audit_metadata.json").write_text(json.dumps(metadata, indent=2, default=str))
    ...
    print(f"Wrote spatial manuscript audit outputs to {OUTPUT_DIR.relative_to(REPO_ROOT)}")
    print(
        compact_permutation_table(permutation)[
            [
                "city",
