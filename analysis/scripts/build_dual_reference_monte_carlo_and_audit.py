...
def audit_phase(phase: int, phase_dir: Path, expectations: dict[str, Any]) -> list[dict[str, Any]]:
...
def build_audit(dual: pd.DataFrame) -> pd.DataFrame:
    rows: list[dict[str, Any]] = []
    for phase, phase_dir in PHASE_DIRS.items():
        rows.extend(audit_phase(phase, phase_dir, PHASE_EXPECTATIONS[phase]))
    rows.append(
        {
            "phase": "dual",
...
def write_dual_summary(summary: pd.DataFrame, audit: pd.DataFrame) -> None:
...
def write_audit_markdown(audit: pd.DataFrame) -> None:
    lines = [
    ...
    for phase_name, frame in audit.groupby("phase_name", sort=False):
        ...
        lines.append("")
    (OUTPUT_DIR / "finite_population_phase_audit.md").write_text("\n".join(lines))
...
def mirror_outputs() -> None:
    for source in [
        ...
        AGGREGATED_DIR / "finite_population_phase_audit.csv",
    ]:
...
def update_readmes(summary: pd.DataFrame, audit: pd.DataFrame) -> None:
    ...
    if packet_readme.exists():
        ...
        for data_line in [
            ...
            "- `data/finite_population_phase_audit.csv`: machine-readable audit of Phases 1–4, seed/config files, selected-subset registries, and expected output files.",
        ]:
...
def main() -> None:
    ...
    audit.to_csv(AGGREGATED_DIR / "finite_population_phase_audit.csv", index=False)
    ...
    write_dual_summary(summary, audit)
    mirror_outputs()
    update_readmes(summary, audit)
